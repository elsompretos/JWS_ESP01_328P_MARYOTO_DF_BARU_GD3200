/* ================================================
  787 Latitude : -6.28
  792 Longitude : 106.70
  802 Timezone : 7.00

  807 adj_subuh : 2    | 32 tpl_subuh : 1    | 49 iqm_subuh : 2
  809 adj_dzuhur : 2   | 34 tpl_dzuhur : 1   | 51 iqm_dzuhur : 2
  811 adj_ashar : 2    | 36 tpl_ashar : 1    | 53 iqm_ashar : 2
  813 adj_maghrib : 2  | 38 tpl_maghrib : 1  | 55 iqm_maghrib : 2
  815 adj_isya : 2     | 40 tpl_isya : 1     | 57 iqm_isya : 2
  817 adj_imsak : 0    | 42 tpl_imsak : 1    | 59 lama_adzan : 4
  819 adj_terbit : 0   | 44 tpl_terbit : 0   |
  821 adj_dhuha : 2    | 46 tpl_dhuha : 0    | 61 lama_imsak : 0
...
void loop() {
  ...
  if (tx_ser.length() > 0) {
    ...
    // Setting Koreksi waktu sholat === KR=02-02-02-02-02-02-00-00 == 02-00-02 imsak dan dhuha
      ...
      EEPROM.put(807, tx_ser.substring(3, 5).toInt());
      EEPROM.put(809, tx_ser.substring(6, 8).toInt());
      EEPROM.put(811, tx_ser.substring(9, 11).toInt());
      EEPROM.put(813, tx_ser.substring(12, 14).toInt());
      EEPROM.put(815, tx_ser.substring(15, 17).toInt());
      EEPROM.put(817, tx_ser.substring(18, 20).toInt());
      EEPROM.put(819, tx_ser.substring(21, 23).toInt());
      EEPROM.put(821, tx_ser.substring(24, 26).toInt());
    ...
    // Setting Lokasi === LL=-6.287617|106.821189|7 === LL=-3.3186|114.5944|8
      ...
      for (int i = 0; i < tx_ser.length(); i++) {
        if (tx_ser.substring(i, i + 1) == "|") {
          ...
          EEPROM.put(787, (tx_ser.substring(3, i)).toDouble());
          EEPROM.put(792, (tx_ser.substring(i + 1, tx_ser.length() - 3)).toDouble());
          EEPROM.put(802, (tx_ser.substring(tx_ser.length() - 2, tx_ser.length())).toDouble());
...
void AMBIL_WAKTU_SHOLAT() {           // AMBIL WAKTU SHOLAT
  ...
  EEPROM.get(787, b_lati);
  EEPROM.get(792, b_longi);
  EEPROM.get(802, b_timezone);

  EEPROM.get(817, b_a[0]); //imsak
  EEPROM.get(807, b_a[1]); //subuh
  EEPROM.get(819, b_a[2]); //syuruq
  EEPROM.get(821, b_a[3]); //dhuha
  EEPROM.get(809, b_a[4]); //dzuhur
  EEPROM.get(811, b_a[5]); //ashar
  EEPROM.get(813, b_a[6]); //maghrib
  EEPROM.get(815, b_a[7]); //isya
