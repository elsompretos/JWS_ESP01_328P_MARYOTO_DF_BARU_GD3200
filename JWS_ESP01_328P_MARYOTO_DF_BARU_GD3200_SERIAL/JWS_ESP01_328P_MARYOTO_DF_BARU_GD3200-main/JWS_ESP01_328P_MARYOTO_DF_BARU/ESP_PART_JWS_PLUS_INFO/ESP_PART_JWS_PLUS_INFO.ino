...
#include "ambil_jadwal.h"


// ==== WIFI ====
const char* ssid = "JWSKU1";
...
void setup() {
  ...
  server.on("/", []() {
    ...
    if (server.hasArg("date")) {
      ...
      if ((server.arg(2)).toInt() < 10) {
        hijri = "0" + server.arg(2);
      } else {
        hijri = server.arg(2);
      }
      if ((server.arg(3)).toInt() < 10) {
        cerah = "0" + server.arg(3);
      } else {
        cerah = server.arg(3);
      }
      if ((server.arg(4)).toInt() < 10) {
        volume_adzan = "0" + server.arg(4);
      } else {
        volume_adzan = server.arg(4);
      }
      if ((server.arg(5)).toInt() < 10) {
        volume = "0" + server.arg(5);
      } else {
        volume = server.arg(5);
      }
      if ((server.arg(6)).toInt() < 10) {
        beep_status = "0" + server.arg(6);
      } else {
        beep_status = server.arg(6);
      }
    ...
    if (server.hasArg("lamaadzan")) {
      ...
      if ((server.arg(0)).toInt() < 10) {
        adzan = "0" + server.arg(0);
      } else {
        adzan = server.arg(0);
      }
      if ((server.arg(1)).toInt() < 10) {
        iqosubuh = "0" + server.arg(1);
      } else {
        iqosubuh = server.arg(1);
      }
      if ((server.arg(2)).toInt() < 10) {
        iqodzuhur = "0" + server.arg(2);
      } else {
        iqodzuhur = server.arg(2);
      }
      if ((server.arg(3)).toInt() < 10) {
        iqoashar = "0" + server.arg(3);
      } else {
        iqoashar = server.arg(3);
      }
      if ((server.arg(4)).toInt() < 10) {
        iqomaghrib = "0" + server.arg(4);
      } else {
        iqomaghrib = server.arg(4);
      }
      if ((server.arg(5)).toInt() < 10) {
        iqoisya = "0" + server.arg(5);
      } else {
        iqoisya = server.arg(5);
      }
    ...
    if (server.hasArg("korimsak")) {
      ...
      if ((server.arg(0)).toInt() < 10) {
        korimsak = "0" + server.arg(0);
      } else {
        korimsak = server.arg(0);
      }
      if ((server.arg(1)).toInt() < 10) {
        korsubuh = "0" + server.arg(1);
      } else {
        korsubuh = server.arg(1);
      }
      if ((server.arg(2)).toInt() < 10) {
        korterbit = "0" + server.arg(2);
      } else {
        korterbit = server.arg(2);
      }
      if ((server.arg(3)).toInt() < 10) {
        kordhuha = "0" + server.arg(3);
      } else {
        kordhuha = server.arg(3);
      }
      if ((server.arg(4)).toInt() < 10) {
        kordzuhur = "0" + server.arg(4);
      } else {
        kordzuhur = server.arg(4);
      }
      if ((server.arg(5)).toInt() < 10) {
        korashar = "0" + server.arg(5);
      } else {
        korashar = server.arg(5);
      }
      if ((server.arg(6)).toInt() < 10) {
        kormaghrib = "0" + server.arg(6);
      } else {
        kormaghrib = server.arg(6);
      }
      if ((server.arg(7)).toInt() < 10) {
        korisya = "0" + server.arg(7);
      } else {
        korisya = server.arg(7);
      }
    ...
    if (server.hasArg("tpl_imsak")) {
      ...
      if ((server.arg(0)).toInt() < 10) {
        tpl_imsak = "0" + server.arg(0);
      } else {
        tpl_imsak = server.arg(0);
      }
      if ((server.arg(1)).toInt() < 10) {
        tpl_subuh = "0" + server.arg(1);
      } else {
        tpl_subuh = server.arg(1);
      }

      if ((server.arg(2)).toInt() < 10) {
        tpl_terbit = "0" + server.arg(2);
      } else {
        tpl_terbit = server.arg(2);
      }
      if ((server.arg(3)).toInt() < 10) {
        tpl_dhuha = "0" + server.arg(3);
      } else {
        tpl_dhuha = server.arg(3);
      }

      if ((server.arg(4)).toInt() < 10) {
        tpl_dzuhur = "0" + server.arg(4);
      } else {
        tpl_dzuhur = server.arg(4);
      }
      if ((server.arg(5)).toInt() < 10) {
        tpl_ashar = "0" + server.arg(5);
      } else {
        tpl_ashar = server.arg(5);
      }
      if ((server.arg(6)).toInt() < 10) {
        tpl_maghrib = "0" + server.arg(6);
      } else {
        tpl_maghrib = server.arg(6);
      }
      if ((server.arg(7)).toInt() < 10) {
        tpl_isya = "0" + server.arg(7);
      } else {
        tpl_isya = server.arg(7);
      }
    ...
    if (server.hasArg("tpl_namamasjid")) {
      ...
      if ((server.arg(0)).toInt() < 10) {
        tpl_namamasjid = "0" + server.arg(0);
      } else {
        tpl_namamasjid = server.arg(0);
      }
      if ((server.arg(1)).toInt() < 10) {
        tpl_info1 = "0" + server.arg(1);
      } else {
        tpl_info1 = server.arg(1);
      }
      if ((server.arg(2)).toInt() < 10) {
        tpl_stlh_iqomah = "0" + server.arg(2);
      } else {
        tpl_stlh_iqomah = server.arg(2);
      }
      if ((server.arg(3)).toInt() < 10) {
        tpl_info2 = "0" + server.arg(3);
      } else {
        tpl_info2 = server.arg(3);
      }
...
void loop() {
  ...
  while (Serial.available() > 0 ) {
    tx_ardu  = Serial.readStringUntil('\r\n');
  }

  if (tx_ardu.length() > 0) {
    
    //RH = Request Hijri RH=31-03-2023-5-1
    if (tx_ardu.substring(0, 2) == "RH") {
      Serial.println(tx_ardu);
      int input_tgl =  tx_ardu.substring(3, 5).toInt();
      int input_bln =  tx_ardu.substring(6, 8).toInt();
      int input_thn =  tx_ardu.substring(9, 13).toInt();
      int input_dow =  tx_ardu.substring(14, 15).toInt();
      int input_adj =  tx_ardu.substring(16, 17).toInt();

      String str_tanggal_hijriyah = Kuwaiti_algorithm(input_tgl, input_bln, input_thn, input_dow, input_adj);
      Serial.println(str_tanggal_hijriyah);
    }

    //RJ = Request Jadwal RJ=31-03-2023
    if (tx_ardu.substring(0, 2) == "RJ") {
      Serial.println(tx_ardu);
      int input_tgl =  tx_ardu.substring(3, 5).toInt();
      int input_bln =  tx_ardu.substring(6, 8).toInt();
      int input_thn =  tx_ardu.substring(9, 13).toInt();

      String str_tanggal_hijriyah = ambil_jadwal(input_tgl, input_bln, input_thn);
      Serial.println(str_tanggal_hijriyah);
    }

    //RT = Request Tanggal RT=31-03-2023-5
    if (tx_ardu.substring(0, 2) == "RT") {
      Serial.println(tx_ardu);
      int input_tgl =  tx_ardu.substring(3, 5).toInt();
      int input_bln =  tx_ardu.substring(6, 8).toInt();
      int input_thn =  tx_ardu.substring(9, 13).toInt();
      ...
      char DayName[7];
      memset(DayName, 0, 7);
      strcpy_P(DayName, namaHariMasehi[input_dow]);
  
      // Bulan Masehi
      char BulanName[12];
      memset(BulanName, 0, 12);
      strcpy_P(BulanName, namaBulanMasehi[input_bln - 1]);

      String str_tanggal_hijriyah = "TR=" +String(DayName) + ", " + String(input_tgl) + " " + String(BulanName) + " " + String(input_thn);
      Serial.println(str_tanggal_hijriyah);
    }
    tx_ardu = "";
  }
...
String ambil_jadwal(double tanggalH, double bulanH, double tahunH) {
  str_tanggal_hijriyah = "JR=";
  
  SholahTimeCalulation sholat;
  sholat.coordinatSet(latitude, longitude, zonawaktu);
  sholat.dateSet(tahunH, bulanH, tanggalH);
